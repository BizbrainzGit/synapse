...
// 	}
 ...
 //       $listresult = self::join('users','users.id','=','user_details.user_id')
	...
	function SearchPatientList(){
		...
		//$search_name,$search_cityid,$customer_mobilenumber,$search_stateid
 ...
 //        if($search_name!=''){
	...
	// 	}
      ...
      $searchresult=Capsule::select(" SELECT user_details.id,CONCAT(user_details.first_name,' ',user_details.last_name) AS name ,users.email,users.active,users.role_id,user_details.mobileno,user_details.id, user_details.user_id, CONCAT(address.houseno ,' ',address.area,' ',address.street,' ',cities.cityname,' ',states.state_name,' ',address.pincode) AS patientaddress, cities.cityname,users.active,
	    ...
	    left join cities on cities.cityid = address.city_id 
	    left join states on states.state_id = address.state_id
		WHERE user_details.id !=0 AND users.role_id=3");     
